...
const transformSessionData = (
	rawSession: BetterAuthSession["session"],
): ActiveSessionSelectAll => {
	return {
		...rawSession,
		...
		markedInvalidAt: rawSession.markedInvalidAt
			? new Date(rawSession.markedInvalidAt).getTime()
			: null,
...
const transformUserData = (
	rawUser: BetterAuthSession["user"],
): UserSelectAll => {
	return {
		...rawUser,
 ...
 * ```
 */
export const transformSessionAndUserData = (
