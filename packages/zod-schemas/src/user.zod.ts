...
import { zString, zTimestampsAsNumbers, zTimezone } from "./zod_utils.js";
...
export const userUpdateInputZod = userMandatoryZod.extend(userOptionalZod.shape).extend(zTimestampsAsNumbers).omit({ email: true }).partial();
export type UserUpdateInput = z.infer<typeof userUpdateInputZod>;

export const userSelectAllZod = userMandatoryZod.extend(userOptionalZod.shape).extend(zTimestampsAsNumbers).extend(userAutoGeneratedZod.shape);
